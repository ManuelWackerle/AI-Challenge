...
class AgentRL_final:
    ...
    def get_move(self, board, score, turns_alive, turns_to_starve, direction, head_position, body_parts):
        agent = 'AgentFinal' + str(len(body_parts))
        if os.path.exists(agent):
            q_object = open(agent, 'rb')
            self.Q = pickle.load(q_object)
            q_object.close()
        """ NOTE ALL AgentFinal PICKLE FILES WERE DELETED BECAUSE THEY DIDN'T WORK AT ALL (and took up way too much space)! """
        self.board = board
        ...
        self.state = state
        int_move = 0
        log = ' '.join(["STATE... values: ", str([state[0], state[1], state[2]]), " q_values:",
                        str(self.Q[state[0]][state[1]][state[2]]), " position", str(self.node_xy(state[0])), "heading",
                        str(direction)])
        logging.info(log)
